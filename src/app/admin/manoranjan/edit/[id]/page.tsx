...
export default function EditManoranjanArticlePage() {
    ...
    fetch(`/api/articles/${id}`)
      ...
      .then((data) => {
        if (data.success && data.data) {
          ...
          setVideoDuration(art.videoDuration || '1:00');
          if (Array.isArray(art.tags)) {
            setTags(art.tags.map((t: any) => t.name || t));
          }
          if (art.sourceType) {
            setSubtype(art.sourceType as any);
          }

          // Parse review metadata from gallery
          if (art.gallery) {
            try {
              const rev = JSON.parse(art.gallery);
              if (rev.movieName) setMovieName(rev.movieName);
              if (rev.rating) setRating(Number(rev.rating));
              if (rev.verdict) setVerdict(rev.verdict);
              if (rev.director) setDirector(rev.director);
              if (rev.cast) setCast(rev.cast);
            } catch {}
          }
        }
      })
      .catch((err) => {
        setError('आर्टिकल लोड करने में त्रुटि हुई');
      })
    ...
    try {
      const res = await fetch(`/api/articles/${id}`, {
        ...
        body: JSON.stringify({
          ...
          featuredImage,
          sourceType: subtype,
          gallery: reviewData ? JSON.stringify(reviewData) : null,
          status,
          isFeatured,
          videoEnabled: Boolean(videoUrl && videoUrl.trim()),
      ...
      if (data.success) {
        ...
        setError(data.error || 'अपडेट करने में समस्या आई');
      }
    } catch (err: any) {
  ...
  if (loading) {
    return (
      <div className="p-12 text-center text-stone-500 text-xs flex items-center justify-center gap-2">
        ...
        <span>आर्टिकल लोड हो रहा है...</span>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-stone-900 flex items-center gap-2">
              ...
              <span>मनोरंजन आर्टिकल संपादित करें</span>
            </h1>
            <p className="text-xs text-stone-500">
              फोटो, रिव्यू रेटिंग, टेक्स्ट व बोलकर लिखने की सुविधा
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {slug && (
            <Link
              href={`/news/${slug}`}
              target="_blank"
              className="px-3 py-2 border border-stone-300 rounded-xl text-xs font-bold text-stone-700 hover:bg-stone-50 flex items-center gap-1"
            >
          ...
          <button
            ...
            disabled={saving}
            className="px-5 py-2 bg-[#EA580C] hover:bg-[#C2410C] text-white rounded-xl text-xs font-black flex items-center gap-2 shadow-sm transition-all active:scale-95 disabled:opacity-50 cursor-pointer"
          >
            <Save className="w-4 h-4" />
            <span>{saving ? 'सुरक्षित हो रहा है...' : 'अपडेट सुरक्षित करें'}</span>
          </button>
        </div>
      </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-black text-stone-900">{st.label}</span>
                    <span
                  ...
                  <p className="text-[10.5px] text-stone-500 mt-1 font-medium">{st.desc}</p>
                </div>
              );
            })}
          </div>
